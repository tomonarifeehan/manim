...
class Title(Scene):  
    def construct(self):
        ...
        titleText = TexText("Example 3", font_size = small_font, color = BLACK)
        self.add(titleText)
...
class Content(Scene):  
    def construct(self):
        ...
        equationTwo_partOne = TexText(latexTwo, font_size = small_font, color = BLACK)
        equationTwo_partOne.shift(0.75 * DOWN + 3.9 * LEFT)

        equationTwo_partTwo = TexText(latexThree, font_size = small_font, color = BLACK)
        equationTwo_partTwo.shift(0.75 * DOWN + 0.7 * LEFT)

        equationTwo_partThree = TexText(latexFour, font_size = small_font, color = BLACK)
        equationTwo_partThree.shift(0.75 * DOWN + 0.9 * LEFT)

        equationTwo_partFour = TexText(latexFive, font_size = small_font, color = BLACK)
        equationTwo_partFour.shift(0.75 * DOWN + 0.3 * LEFT)

        # g'(x) = 2x * cos(x^2 + 3x + 4) + 3 * cos(x^2 + 3x + 4)
        equationThree_partOne = TexText(latexSix, font_size = small_font, color = BLACK)
        equationThree_partOne.shift(1.75 * DOWN + 3.1 * LEFT)

        equationThree_partTwo = TexText(latexSeven, font_size = small_font, color = BLACK)
