...
class Content(Scene):  
    def construct(self):
        ...
        textbox = create_textbox(height = 5, width = 8, color = TEAL_B, string = "")
        self.add(textbox)

        textbox.shift(1 * DOWN + 2.5 * LEFT)

class ChainRule(Scene):
    def construct(self):
        
        # h(x) = f[g(x)]
        equationOne = TexText(latexOne)
        equationOne.shift(0.5 * UP + 4 * LEFT)
        
        # h'(x) = f'[g(x)] * g'(x)
        equationTwo_partOne = TexText(latexTwo_partOne)
        equationTwo_partOne.shift(1 * DOWN + 3.7 * LEFT)

        equationTwo_partTwo= TexText(latexTwo_partTwo)
        equationTwo_partTwo.shift(1 * DOWN + 0.6 * LEFT)
